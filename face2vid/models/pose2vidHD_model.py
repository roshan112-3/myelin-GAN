...
# (input_nc = 27, output_nc = 3, ngf = 32, netG = local, n_downsample_global = 4, n_blocks_global = 9, n_local_enhancers = 1, n_blocks_local =3, instance, 0) 
    ...
    def forward(self, label, real_image):
        ...
        # x1 = torch.cat((label[:,0, ...], label[:,1, ...], label[:,2, ...], label[:,3, ...], label[:,4, ...]), dim=1)
        x1 = torch.cat((label[:,0, ...], label[:,1, ...], label[:,2, ...], label[:,3, ...], label[:,4, ...], label[:,5, ...], label[:,6, ...], label[:,7, ...], label[:,8, ...], label[:,9, ...]), dim=1)
        y1 = self.netG.forward(x1)
        
        # Fake Detection and Loss
        pred_fake_pool = self.netD.forward(torch.cat((x1, y1.detach()), dim=1))
        loss_D_fake = self.criterionGAN(pred_fake_pool, False)        

        # Real Detection and Loss        
        pred_real = self.netD.forward(torch.cat((x1, gt1.detach()), dim=1))
        loss_D_real = self.criterionGAN(pred_real, True)

        # GAN loss (Fake Possibility Loss)
        pred_fake = self.netD.forward(torch.cat((x1, y1), dim=1))
            ...
            for i in range(self.opt.num_D):
                for j in range(len(pred_fake[i])-1):
                    loss_G_GAN_Feat += D_weights * feat_weights * \
                        self.criterionFeat(pred_fake[i][j], pred_real[i][j].detach()) * self.opt.lambda_feat * 0.2
                    
        # VGG feature matching loss
        loss_G_VGG = 0
        if not self.opt.no_vgg_loss:
            loss_G_VGG = (self.criterionVGG(y1, gt1))\
                            * self.opt.lambda_feat

        # 20181012: pwc-flow matching loss
        loss_G_flow = self.criterionFeat(y1, gt1) * self.opt.lambda_flow
        # if not self.opt.no_flow_loss:
        #     # loss_G_flow = self.criterionFlow(gt1.detach(), gt2.detach(), y1, y2) * self.opt.lambda_flow / self.nelem
        #     loss_G_flow = self.criterionFlow(gt1.detach(), gt1.detach(), y1, y1) * self.opt.lambda_flow
    ...
    def inference(self, label, inst=None, prev_frame=None):
        ...
        # input_concat = torch.cat((input_label[:,0, ...], input_label[:,1, ...], input_label[:,2, ...], input_label[:,3, ...], input_label[:,4, ...]), dim=1)
        input_concat = torch.cat((input_label[:,0, ...], input_label[:,1, ...], input_label[:,2, ...], input_label[:,3, ...], input_label[:,4, ...], input_label[:,5, ...], input_label[:,6, ...], input_label[:,7, ...], input_label[:,8, ...], input_label[:,9, ...]), dim=1)
